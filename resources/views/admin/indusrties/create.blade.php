...
<x-app-layout>
    <x-slot:header>
        <div class="w-full flex items-center justify-between">
            ...
            <a href="{{ route('indusrties.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center">{{ __('К списку заявок') }}</a>
        </div>
        </x-slot>
        <div class="my-4 p-4 rounded-md max-w-3xl mx-auto bg-white">
            <x-splade-form action="{{route('indusrties.store')}}">
                ...
                <x-splade-file name="back_img" label="{{__('ФИО')}}" placeholder="{{__('Имя')}}"/>
